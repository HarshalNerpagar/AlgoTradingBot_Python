...
def calculate_td_support_resistance(data):
    ...
    for i in range(1, len(data)):
        ...
        if data['buy_setup'][i] > 0 and data['buy_setup'][i] % 1 == 0:
            data.at[i, 'support_level'] = min(data['low'][i :i + 1])
            # print(f"Candle {i}" , "Support -> ",data['support_level'][i])
        # if data['sell_setup'][i] == 6:
        if data['sell_setup'][i] > 0 and data['sell_setup'][i] % 1 == 0:
            data.at[i, 'resistance_level'] = max(data['high'][i :i + 1])
            # print(f"Candle {i}" , "Resistance -> " , data['resistance_level'][i])

    return data

def generate_trade_signals(data, levels, long=False, short=False, start=False):
    ...
    for i in range(len(data)):
        ...
        if curr_time == '09:15:00' or start:
            support_level = 0
            resistance_level = float('inf')
